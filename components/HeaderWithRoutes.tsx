...
import { navigationRoutes, contact } from '@/data/content';
import { FaPhone, FaEnvelope } from 'react-icons/fa';
import Link from 'next/link';
import { usePathname } from 'next/navigation';

export default function HeaderWithRoutes() {
  ...
  const isActive = (href: string) => {
    ...
    if (href.startsWith('/#')) {
      ...
      return pathname === '/' && false; // Will be handled by scroll detection
    }
  ...
  return (
    <>
      ...
      <div className={`hidden lg:block bg-primary-900 text-white transition-all duration-300 ${
      ...
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          isScrolled
            ...
            : 'bg-white/90 backdrop-blur-sm py-2 lg:mt-10'
        }`}
      >
        <nav className="container-custom">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden lg:block">
              <Link
                href="/kontakt"
                className="relative overflow-hidden group px-6 py-3 bg-gradient-to-r from-primary-500 to-primary-600 hover:from-primary-600 hover:to-primary-700 text-white font-semibold rounded-lg transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 inline-flex items-center space-x-2"
              >
      ...
      >
        ...
        >
          ...
          >
            <a href={`tel:${contact.phone}`} className="flex items-center space-x-2 hover:text-accent-400 transition-colors">
              <FaPhone className="w-4 h-4" />
              <span>{contact.phone}</span>
            </a>
            <a href={`mailto:${contact.email}`} className="flex items-center space-x-2 hover:text-accent-400 transition-colors">
